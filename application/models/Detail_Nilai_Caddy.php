...
class Detail_Nilai_Caddy extends CI_Model
{
    var $select = '*';
    var $table1 = 't_absen';
    ...
    private function _get_datatables_query($fp, $tgl)
    {

        $this->db->select($this->select);
        $this->db->from($this->table1);
        $this->db->where(['stat' => 'M', 'stat_nilai' => 'BN', 'id_fingerprint' => $fp]);
        $this->db->like('tanggal', $tgl, 'both');
    ...
    function get_datatables($fp, $tgl)
    {
        $this->_get_datatables_query($fp, $tgl);
        if ($_POST['length'] != -1)
    ...
    function count_filtered($fp, $tgl)
    {
        $this->_get_datatables_query($fp, $tgl);
        $query = $this->db->get();
    ...
    public function count_all($fp, $tgl)
    {
        $this->db->select($this->select);
        $this->db->from($this->table1);
        $this->db->where(['stat' => 'M', 'stat_nilai' => 'BN', 'id_fingerprint' => $fp]);
